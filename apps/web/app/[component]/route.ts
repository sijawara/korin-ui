...
const packageDir = join(process.cwd(), "..", "..", "packages", "korin-ui");
const packagePath = join(packageDir, "package.json");
const packageJson = JSON.parse(await readFile(packagePath, "utf-8"));

const examplesDir = join(process.cwd(), "..", "..", "packages", "korin-example", "src");

const internalDependencies = Object.keys(packageJson.dependencies || {}).filter(
  (dep) => dep.startsWith("@monorepo") && dep !== "@monorepo/shadcn-ui",
);

const dependenciesSet = new Set(
  Object.keys(packageJson.dependencies || {}).filter(
    (dep) => !["react", "react-dom", "@monorepo/shadcn-ui", ...internalDependencies].includes(dep),
  ),
);

const devDependenciesSet = new Set(
  Object.keys(packageJson.devDependencies || {}).filter(
    (dep) => !["@monorepo/tsconfig", "@types/react", "@types/react-dom", "typescript"].includes(dep),
  ),
);
...
const exampleTsxFiles = exampleFiles.filter((file) => file.isFile() && file.name.endsWith(".tsx"));
...
const fileContents = await Promise.all(
  tsxFiles.map(async (tsxFile) => {
    ...
    const parsedContent = content
      .replace(/@monorepo\/shadcn-ui\//g, "@/")
      .replace(/@monorepo\/ui\//g, "@/components/korin-ui/");

    return {
      type: "registry:component",
      path: `registry/default/korin-ui/${tsxFile.name}`,
...
const korinUIComponents =
  files
    .map((f) => f.content)
    .join("\n")
    .match(/@\/components\/korin-ui\/([a-z-]+)/g)
    ?.map((path) => path.split("/").pop())
    .filter((name): name is string => Boolean(name)) || [];
    ...
    files: [
      {
        ...
        type: "registry:component",
        target: `components/korin-ui/${componentFile.name}.tsx`,
      },
    ],
  };
    ...
    files: [
      {
        path: `registry/default/examples/${exampleFile.name}`,
        type: "registry:block",
        target: `components/korin-ui/examples/${exampleFile.name}.tsx`,
      },
    ],
  };
...
const response: Registry = {
  name: "korin-ui",
  homepage: new URL("/", registryUrl).toString(),
  items,
};
...
export const GET = async (_request: NextRequest, { params }: RequestProps) => {
  ...
  if (parsedComponent === "all") {
    ...
    for (const componentFile of tsxFiles) {
      const file = files.find((f) => f.path === `registry/default/korin-ui/${componentFile.name}`);
      ...
      if (file) {
        allFiles.push({
          ...
          target: `components/korin-ui/${componentFile.name}`,
        });
  ...
  if (!item) {
    return NextResponse.json({ error: `Component "${parsedComponent}" not found.` }, { status: 404 });
  }
  ...
  if (item.type === "registry:component") {
    file = files.find((f) => f.path === `registry/default/korin-ui/${parsedComponent}.tsx`);
    ...
    file = files.find((f) => f.path === `registry/default/examples/${exampleFileName}`);
  }

  if (!file) {
    return NextResponse.json({ error: `File for "${parsedComponent}" not found.` }, { status: 404 });
  }
  ...
  for (const dep of internalDependencies) {
    const packageName = dep.replace("@monorepo/", "");
    usedRegistryDependencies.add(new URL(`/${packageName}.json`, registryUrl).toString());
  }
    ...
    files: [
      {
        ...
        type: file.type as RegistryItem["type"],
        content: file.content,
        target: `components/korin-ui/${item.name}.tsx`,
      },
    ],
